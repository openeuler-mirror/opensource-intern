...
function getRightInfo(cell) {
    ...
    if (cell && cell.style && (cell.style).indexOf("image\;") === 0 && processType == "GROUP") {
        $.ajax({
            cache: true,
            type: "POST",
            url: "/openEuler-web/processGroup/queryProcess",
            data: data,
            async: true,
        ...
        if (processType == "GROUP") {
            $.ajax({
                cache: true,
                type: "POST",
                url: " /openEuler-web/processGroup/queryProcessGroup",
                data: data,
                async: true,
                error: function (request) {
                    return;
            ...
            $.ajax({
                cache: true,
                type: "POST",
                url: "/openEuler-web/page/process/inc/process_info_inc.html",
        ...
        $.ajax({
            cache: true,
            type: "POST",
            url: "/openEuler-web/page/process/inc/process_property_inc.html",
        ...
        if (processType == "GROUP") {
            $.ajax({
                cache: true,
                type: "POST",
                url: "/openEuler-web/processGroup/queryProcessGroupPath",
            ...
            $.ajax({
                cache: true,
                type: "POST",
                url: "/openEuler-web/page/process/inc/process_path_inc.html",
                data: {processId: processGroupId, pageId: cell.id},
                async: true,
                error: function (request) {
                    return;
        ...
        if (EditorUi.prototype.noEditing) {
            setTimeout(() => {
                ...
                nodeArr.forEach(item => {
                    ...
                    img_element_init.setAttribute("openEuler_IMG", "IMG");
                    img_element_init.href.baseVal = "/openEuler-web/img/Loading.gif";
                    ...
                    img_element_ok.setAttribute("openEuler_IMG", "IMG");
                    img_element_ok.href.baseVal = "/openEuler-web/img/Ok.png";
                    ...
                    img_element_fail.setAttribute("openEuler_IMG", "IMG");
                    img_element_fail.href.baseVal = "/openEuler-web/img/Fail.png";
...
function prohibitEditing(evt, operationType) {
    ...
    $.ajax({
        ...
        url: "/openEuler-web/mxGraph/eraseRecord",
        data: {},
        async: true,
...
function groupGraphAddCells(cells) {
    ...
    if (cells.length != removeCellArray.length) {
        ...
        $.ajax({
            cache: true,//Keep cached data
            type: "POST",//Request type post
            url: "/openEuler-web/mxGraph/addMxCellAndData",
            data: JSON.stringify({
...
function openProcessMonitor(evt) {
    ...
    if (cellFor.style && (cellFor.style).indexOf("text\;") === 0) {
        ...
        $.ajax({
            cache: true,
            type: "POST",
            url: "/openEuler-web/flowGroup/findflowByGroup",
            ...
            success: function (data) {
                ...
                if (200 === dataMap.code) {
                    if ('flow' === dataMap.nodeType) {
                        ...
                        window.location.href = "/openEuler-web/mxGraph/drawingBoard?drawingBoardType=TASK&parentAccessPath=flowGroupList&load=" + flow_obj.id;
                    } else if ('flowGroup' === dataMap.nodeType) {
                        var flowGroup_obj = dataMap.flowGroupVo;
                        window.location.href = "/openEuler-web/mxGraph/drawingBoard?drawingBoardType=GROUP&parentAccessPath=flowGroupList&load=" + flowGroup_obj.id;
                    }
                } else {
...
function OpenTheMonitorArtboard(evt) {
    ...
    if (cellFor.style && (cellFor.style).indexOf("text\;") === 0) {
        ...
        $.ajax({
            cache: true,//Keep cached data
            type: "POST",//Request type post
            url: "/openEuler-web/processGroup/getProcessIdByPageId",//This is the name of the file where I receive data in the background.
            data: {
            ...
            success: function (data) {//Operation after request successful
                if (data) {
                    ...
                    if (200 === dataMap.code) {
                        var urlPath = "";
                        // if ('flow' === dataMap.nodeType) {
                        //     urlPath = "/openEuler-web/mxGraph/drawingBoard?drawingBoardType=PROCESS&processType=PROCESS&load=" + dataMap.processId;
                        // } else if ('flowGroup' === dataMap.nodeType) {
                        //     urlPath = "/openEuler-web/mxGraph/drawingBoard?drawingBoardType=PROCESS&processType=PROCESS_GROUP&load=" + dataMap.processGroupId;
                        // }
                        if ('flow' === dataMap.nodeType) {
                            urlPath = "/openEuler-web/mxGraph/drawingBoard?drawingBoardType=PROCESS&parentAccessPath=processGroupList&processType=PROCESS&load=" + dataMap.processId;
                        } else if ('flowGroup' === dataMap.nodeType) {
                            urlPath = "/openEuler-web/mxGraph/drawingBoard?drawingBoardType=PROCESS&parentAccessPath=processGroupList&processType=PROCESS_GROUP&load=" + dataMap.processGroupId;
                        }
                        if (urlPath) {
...
function queryStopsProperty(stopPageId) {
    $.ajax({
        cache: true,
        type: "POST",
        url: "/openEuler-web/stops/queryIdInfo",
            ...
            if (200 === dataMap.code) {
                ...
                if (oldPropertiesVo && oldPropertiesVo.length > 0) {
                    ...
                    for (var y = 0; y < oldPropertiesVo.length; y++) {
                        ...
                        img.setAttribute('src', '/openEuler-web/img/descIcon.png');
...
function queryflowOrflowGroupProperty(flowPageId) {
    ...
    $.ajax({
        cache: true,
        type: "POST",
        url: "/openEuler-web/flowGroup/queryIdInfo",
...
function queryPathInfo(id) {
    ...
    if ('TASK' === Format.customizeType) {
        param_values.url = "/openEuler-web/path/queryPathInfo";
        param_values.customizeBasic_td_2_1_span_children = 'flowName：';
    } else if ('GROUP' === Format.customizeType) {
        param_values.url = "/openEuler-web/flowGroupPath/queryPathInfoflowGroup";
    ...
    if (addParamData.data && addParamData.data != null && addParamData.data.length > 0) {
        ...
        var btn = mxUtils.button('submit', mxUtils.bind(this, function (evt) {
            ...
            $.ajax({
                cache: true,
                type: "POST",
                url: "/openEuler-web/stops/updateStops",
        ...
        var dataSource = '<table style="border-collapse: separate; border-spacing: 0px 5px; margin-left: 12px; width: 97%;">'
            ...
            + '<td style="width: 25px;"><img src="/openEuler-web/img/descIcon.png" title="Fill Datasoure" style="cursor: pointer;"></td>'
            + '<td>'
...
function setCustomizedTableHtml(stopPageId, stopsCustomizedPropertyVo, stopOutPortType) {
    ...
    if (stopsCustomizedPropertyVo) {
        table_tr = '<tr class="trTableCustomizedStopProperty">'
            ...
            + '</td>'
            + '<td style="width: 25px;">'
            + '<img src="/openEuler-web/img/descIcon.png" title="' + stopsCustomizedPropertyVo.description + '" style="cursor: pointer;">'
            + '</td>'
            + '<td>'
...
function getDatasourceList(stop_id, stop_page_id, dataSourceVo) {
    $.ajax({
        ...
        url: "/openEuler-web/datasource/getDatasourceList",
...
function fillDatasource(datasource, stop_id, stop_page_id) {
    ...
    if (stop_id) {
        $.ajax({
            cache: true,//Keep cached data
            type: "POST",//Request type post
            url: "/openEuler-web/datasource/fillDatasource",//This is the name of the file where I receive data in the background.
...
function saveXml(paths, operType) {
    ...
    var url = "/openEuler-web/mxGraph/saveDataForTask";
    if ('TASK' === Format.customizeType) {
        url = "/openEuler-web/mxGraph/saveDataForTask";
    } else if ('GROUP' === Format.customizeType) {
        url = "/openEuler-web/mxGraph/saveDataForGroup";
    }
    var time, time1
    $.ajax({
        cache: true,//Keep cached data
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.ajax({
                    cache: true,
                    type: "POST",
                    url: "/openEuler-web/flowGroup/updateflowGroupBaseInfo",
                    data: {
                        id: flowGroupdata.id,
...
function saveflow() {
    ...
    if (!checkflowInput(flowName, description, driverMemory, executorNumber, executorMemory, executorCores)) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.ajax({
                    cache: true,
                    type: "POST",
                    url: "/openEuler-web/flow/updateflowBaseInfo",
                    data: {
                        id: flowdatas.id,
        ...
        //     // url: "/openEuler-web/flow/saveflowInfo",//This is the name of the file where I receive data in the background.
        //     //
        ...
        //                     alert('The window cannot be opened. Please check your browser settings.')
        //                 } else {
        //                     tempWindow.location = "/openEuler-web/mxGraph/drawingBoard?drawingBoardType=TASK&load=" + dataMap.flowId;
        //                 }
        //             });
        //         } else {
        //             layer.msg('creation failed', {icon: 2, shade: 0, time: 2000}, function () {
...
function openXml() {
    $.ajax({
        cache: true,//Keep cached data
        type: "POST",//Request type post
        url: "/openEuler-web/flow/loadData",
        //data:$('#loginForm').serialize(),//Serialize the form
        async: true,//Synchronous Asynchronous
        error: function (request) {//Operation after request failure
            return;
...
function reloadStops() {
    ...
    $.ajax({
        ...
        cache: true,//Keep cached data
        type: "POST",//Request type post
        url: "/openEuler-web/stops/reloadStops",
        error: function (request) {//Operation after request failure
            // fullScreen.hide();
            window.parent.postMessage(false);
            //alert("reload fail");
        ...
        success: function (data) {//Operation after request successful
            var dataMap = JSON.parse(data);
            if (200 === dataMap.code) {
                window.location.href = "/openEuler-web/mxGraph/drawingBoard?drawingBoardType=TASK&load=" + dataMap.load + "&_" + new Date().getTime();
            } else {
                //alert("reload fail");
                layer.msg("reload fail", {icon: 2, shade: 0, time: 2000});
                // fullScreen.hide();
...
function queryflowInfo() {
    $.ajax({
        ...
        cache: true,//Keep cached data
        type: "POST",//Request type post
        url: "/openEuler-web/flow/queryflowData",
        async: true,//Synchronous Asynchronous
        error: function (request) {//Operation after request failure
            return;
        },
...
function queryflowGroup() {
    $.ajax({
        ...
        cache: true,//Keep cached data
        type: "POST",//Request type post
        url: "/openEuler-web/flowGroup/queryflowGroupData",
        async: true,//Synchronous Asynchronous
        error: function (request) {//Operation after request failure
            return;
        },
...
function runflow(runMode) {
    ...
    $.ajax({
        cache: true,//Keep cached data
        type: "POST",//Request type post
        url: "/openEuler-web/flow/runflow",
        //data:$('#loginForm').serialize(),//Serialize the form
        data: data,
        async: true,//Synchronous Asynchronous
        error: function (request) {//Operation after request failure
            ...
            if (200 === dataMap.code) {
                layer.msg(dataMap.errorMsg, {icon: 1, shade: 0, time: 2000}, function () {
                    ...
                    if (tempWindow == null || typeof (tempWindow) == 'undefined') {
                        ...
                        tempWindow.location = "/openEuler-web/mxGraph/drawingBoard?drawingBoardType=PROCESS&processType=PROCESS&load=" + dataMap.processId;
                    }
                });
            } else {
...
function runflowGroup(runMode) {
    ...
    $.ajax({
        cache: true,//Keep cached data
        type: "POST",//Request type post
        url: "/openEuler-web/flowGroup/runflowGroup",
        //data:$('#loginForm').serialize(),//Serialize the form
        data: data,
        async: true,//Synchronous Asynchronous
        error: function (request) {//Operation after request failure
        ...
        success: function (data) {//After the request is successful
            ...
            if (200 === dataMap.code) {
                layer.msg(dataMap.errorMsg, {icon: 1, shade: 0, time: 2000}, function () {
                    ...
                    var windowOpen = window.open("/openEuler-web/mxGraph/drawingBoard?drawingBoardType=PROCESS&processType=PROCESS_GROUP&load=" + dataMap.processGroupId, '_blank');
                    //var tempwindow = window.open('_blank');
    ...
    if (paths && paths.length > 0) {
        ...
        if (pathsCells.length > 1) {
            ...
            $.ajax({
                cache: true,
                type: "get",
                url: "/openEuler-web/stops/getStopsPort",
                data: {
                    "flowId": loadId,
    ...
    if (pathsCells.length > 1) {
        ...
        if (reqData) {
            $.ajax({
                cache: true,
                type: "get",
                url: "/openEuler-web/path/savePathsPort",
                data: reqData,
                async: true,
                traditional: true,
                error: function (request) {
...
function saveCheckpoints(stopId) {
    ...
    $.ajax({
        cache: true,
        type: "POST",
        url: "/openEuler-web/stops/updateStopsById",
        data: {
            stopId: stopId,
...
function saveTemplateFun(url) {
    ...
    layer.prompt({
        ...
        $.ajax({
            cache: true,//Keep cached data
            type: "POST",//Request type post
            url: "/openEuler-web/flowTemplate/saveflowTemplate",
            data: {
...
function uploadTemplateFile(element) {
    ...
    $.ajax({
        type: 'post',
        url: '/openEuler-web/flowTemplate/uploadXmlFile',
        data: formData,
        cache: false,
        processData: false,
        contentType: false,
...
function loadingXml(id, loadId) {
    ...
    $.ajax({
        ...
        async: true,
        url: "/openEuler-web/flowTemplate/loadingXmlPage",
    }).success(function (data) {
        var dataMap = JSON.parse(data);
        var icon_code = 2;
        if (200 === dataMap.code) {
...
function openTemplateList() {
    ...
    $.ajax({
        url: "/openEuler-web/flowTemplate/flowTemplateList",
        type: "post",
        async: false,
        success: function (data) {
            var dataMap = JSON.parse(data);
...
function getRunningProcessList() {
    $.ajax({
        cache: true,//Keep cached data
        type: "POST",//Request type post
        url: "/openEuler-web/process/getRunningProcessList",
        data: {"flowId": loadId},
        async: true,
        error: function (request) {//Operation after request failure
            return;
...
function addStopCustomProperty(reqData) {
    $.ajax({
        type: "POST",//Request type post
        url: "/openEuler-web/stops/addStopCustomizedProperty",//This is the name of the file where I receive data in the background.
        data: reqData,
        error: function (request) {//Operation after request failure
            return;
        },
...
function removeStopCustomProperty(stopPageId, customPropertyId, isRouter) {
    if (isRouter) {
        ...
        var reqUrl = "/openEuler-web/stops/deleteStopsCustomizedProperty";
...
function getRouterAllPaths(customPropertyId) {
    var reqUrl = "/openEuler-web/stops/getRouterStopsCustomizedProperty";
    var reqData = {customPropertyId: customPropertyId};
    $.ajax({
        type: "POST",//Request type post
        url: reqUrl,//This is the name of the file where I receive data in the background.
...
function removeRouterStopCustomProperty(customPropertyId) {
    var reqUrl = "/openEuler-web/stops/deleteRouterStopsCustomizedProperty";
    var reqData = {customPropertyId: customPropertyId};
    $.ajax({
        type: "POST",//Request type post
        url: reqUrl,//This is the name of the file where I receive data in the background.
...
function openDatasourceList() {
    ...
    $.ajax({
        type: "POST",//Request type post
        url: "/openEuler-web/page/dataSource/getDatasourceListPage",
        error: function (request) {//Operation after request failure
            return;
        },
        success: function (data) {//Operation after request successful
...
function getflowList() {
    ...
    $.ajax({
        type: "POST",//Request type post
        url: "/openEuler-web/page/flow/getflowListHtml",//This is the name of the file where I receive data in the background.
        error: function (request) {//Operation after request failure
            return;
        },
        success: function (data) {//Operation after request successful
...
function changIconTranslate() {
    var iconPositionElementArr = document.querySelectorAll("div[style='display: inline-block; font-size: 1px; font-family: openEuler; color: #FFFFFF; line-height: 1.2; pointer-events: all; white-space: nowrap; ']");
    if (iconPositionElementArr && iconPositionElementArr.length > 0) {
        ...
        var imgsArr = document.querySelectorAll("image[openEuler_IMG='IMG']");
...
function deleteLastReloadData(stopId) {
    $.ajax({
        type: "POST",//Request type post
        url: "/openEuler-web/stops/deleteLastReloadData",//This is the name of the file where I receive data in the background.
        data: {stopId: stopId},
        error: function (request) {//Operation after request failure
            return;
        },
