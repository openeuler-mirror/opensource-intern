...
void zt_log(bool err, const char *format, ...)
{
    ...
    vsnprintf(sbuf, 100, format, args);
    va_end(args);

    sprintf(command,
            "echo '%s [ZerotrustCc] %s: %s' >> %s",
            timestamp,
            err ? "ERROR" : "LOG",
            sbuf,
            LOG_FILE);
...
/*
 * Using Socket to check traffic
 */
int check_traffic()
{
    int sock = 0;
    if ((sock = socket(AF_INET, SOCK_STREAM, 0)) < 0)
        errno_exit("Socket creation");

    struct sockaddr_in serv_addr;
    serv_addr.sin_family = AF_INET;
    serv_addr.sin_port = htons(TI_PORT);
    inet_pton(AF_INET, "127.0.0.1", &serv_addr.sin_addr);
    if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0)
        errno_exit("Connect traffic server");
...
void PostgresInitializer::ZerotrustCheck()
{
    if(!getenv("ZT_CHECK"))
        return;
    
    char *zt_env = getenv("ZT_CHECK");

    /* check role of current user, root should not be checked */
    if (!strcmp(zt_env, "YES") && !superuser_arg(get_role_oid(m_username, false)))
    {
        zt_log(false, "Current user: %s -> NOT root -> %s", m_username, m_indbname);
        write_stderr("LOG: Enter Zerotrust function\n");
        /* Capture images of current user */
        int level = 0, ret;
        CaptureVideo cv;
        char command[100] = {0};
        cv.init();
        ...
        sprintf(command,
                "python3 $CODE_BASE/src/zerotrust/emotion-classify/apply.py %s",
                m_username);
        if((ret = zt_exec(command)) < 0)
        {
            zt_log(true, "Could not check user's emotion");
            exit(EXIT_FAILURE);
        }
        if(!ret)
            /* abnormal emotion, revoke level up */
            level += 1;
        
        if((ret = check_traffic()) < 0)
        {
            zt_log(true, "Could not check traffic");
            exit(EXIT_FAILURE);
        }
        if(!ret)
            /* abnormal traffic, revoke level up */
            level += 1;
        
        if (level) {
            CLEAR(command);
            sprintf(command,
                    "python3 $CODE_BASE/src/zerotrust/privilege-revoke/main.py %s %s %d",
                    m_indbname,
                    m_username,
                    level);
            if(zt_exec(command) < 0)
            {
                zt_log(true, "Could not revoke privileges");
                exit(EXIT_FAILURE);
            }
        }
        zt_log(false, "ZerotrustCheck finished, revoke level is %d.", level);
    }
}
...
void CaptureVideo::init_device(void)
{
    ...
    if (fd == -1)
    {
        zt_log(true, "Cannot open '%s': %d, %s\n", dev_name, errno, strerror(errno));
        exit(EXIT_FAILURE);
    }

    /* 输出摄像机型号参数 */
    ioctl(fd, VIDIOC_QUERYCAP, &cap); // 驱动信息保存在cap结构体
    zt_log(false, "Driver Name: %s, Card Name: %s, Bus info: %s.", cap.driver, cap.card, cap.bus_info);
    ...
    if (xioctl(fd, VIDIOC_QUERYCAP, &cap) == -1)
    {
        if (errno == EINVAL)
        {
            zt_log(true, "%s is no V4L2 device\n", dev_name);
            exit(EXIT_FAILURE);
        }
        else
    ...
    {
        zt_log(true, "%s is no video capture device\n", dev_name);
        exit(EXIT_FAILURE);
    }

    if (!(cap.capabilities & V4L2_CAP_STREAMING))
    {
        zt_log(true, "%s does not support streaming i/o\n", dev_name);
        exit(EXIT_FAILURE);
    }
}

/* 设置图像格式 */
void CaptureVideo::set_img_fmt()
{
    ...
    if (IMAGE_OR_VIDEO)
    {
        ...
        parm.parm.capture.timeperframe.denominator = VIDEO_FRAME_RATE;
...
/* 申请内核缓冲队列并映射到用户空间 */
void CaptureVideo::apply_img_buf()
{
    ...
    if (xioctl(fd, VIDIOC_REQBUFS, &req) == -1)
    {
        if (errno == EINVAL)
        {
            zt_log(true, "%s does not support memory mapping\n", dev_name);
            exit(EXIT_FAILURE);
        }
        else
    ...
    if (req.count < 2)
    {
        zt_log(true, "Insufficient buffer memory on %s\n", dev_name);
        exit(EXIT_FAILURE);
    }

    buffers = new video_buffer[req.count];
    if (!buffers)
    {
        zt_log(true, "Out of memory\n");
        exit(EXIT_FAILURE);
    }
    ...
    for (n_buffers = 0; n_buffers < req.count; ++n_buffers)
    {
        ...
        /* 通过mmap建立映射关系, 把内核空间中的图像缓冲区映射到用户空间 */
...
void CaptureVideo::start_capture()
{
    enum v4l2_buf_type type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    if (xioctl(fd, VIDIOC_STREAMON, &type) == -1)
        errno_exit("VIDIOC_STREAMON");
    if ((access(IMAGE_PATH, F_OK)) == -1)
        if (mkdir(IMAGE_PATH, S_IRWXU) < 0)
        {
            zt_log(true, "Can't make folder %s error %d, %s", IMAGE_PATH, errno, strerror(errno));
            exit(EXIT_FAILURE);
        }
    if (IMAGE_OR_VIDEO)
        save_to_video();
...
/* 保存帧为图像 */
void CaptureVideo::save_to_images()
{
    struct v4l2_buffer buf;
    buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    buf.memory = V4L2_MEMORY_MMAP;
    ...
    int file_fd;
    char filename[20] = {0};
    zt_log(false, "Start to save images");
    for (int i = 1; i <= FRAME_COUNT; i++)
    {
        sprintf(filename, "%s/%d.jpg", IMAGE_PATH, i);
        file_fd = open(filename, O_RDWR | O_CREAT, 0777); // 用来保存图片
        if (file_fd < 0)
        {
            zt_log(true, "Cannot create file '%s': %d, %s", filename, errno, strerror(errno));
            exit(EXIT_FAILURE);
        }
        /* Frame out queue */
...
/* 保存帧为视频 */
void CaptureVideo::save_to_video()
{
    struct v4l2_buffer buf;
    buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    buf.memory = V4L2_MEMORY_MMAP;
    ...
    int file_fd;
    char filename[20] = {"tmp/video.yuv"};
    file_fd = open(filename, O_RDWR | O_CREAT, 0777); // 用来保存视频
    if (file_fd < 0)
    {
        zt_log(true, "Cannot create file '%s': %d, %s", filename, errno, strerror(errno));
        exit(EXIT_FAILURE);
    }
    zt_log(false, "Start to saving video");
    for (int i = 1; i <= FRAME_COUNT; i++)
    {
        /* Frame out queue */
        if (xioctl(fd, VIDIOC_DQBUF, &buf) == -1)
        ...
        write(file_fd, buffers[buf.index].start, buf.bytesused);
...
void CaptureVideo::stop_capture()
{
    ...
    close(fd);
    delete[] buffers;
    zt_log(false, "Camera Done.");
}

void CaptureVideo::init()
{
    init_device();
    set_img_fmt();
    apply_img_buf();
